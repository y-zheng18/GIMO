...
class Encoder(nn.Module):
    ...
    def __init__(self, n_layers, n_head, d_k, d_v, d_model, d_inner, dropout=0.1, norm=False, n_position=400):
...
if __name__ == '__main__':
    ...
    feats = torch.randn((32, 4, 768)).to(device) # (bs, len, dim)

    transformer = Transformer(n_layers=3, n_head=12, d_model=768, d_v=64, d_k=64, d_inner=2048).to(device)
    feats_baseline = torch.mean(feats, dim=1, keepdim=True).to(device)
    feats_fused = transformer(feats, feats_baseline)   # (bs * num_points, 1, dim)
